    ...
    public class JournalsController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("ID,Title,Year")] Journal journal)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var authors = _context.Author.Find(user.AuthorID);
                authors.Journals.Add(journal);
                await _context.SaveChangesAsync();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("ID,Title,Year,AuthorID")] Journal journal)
        {
            if (id != journal.ID)
            {
                return NotFound();
